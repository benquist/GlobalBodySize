...
# Pre-built taxon lists shipped with the app (data/taxon_*.rds), generated by
# tools/build_taxon_lists.R. Loaded once at startup; serves as the primary
# source for autocomplete. Live BIEN SQL is only used as a fallback if the
# RDS files are missing. This avoids runtime LIMIT truncation and silent
# demotion to the tiny hardcoded list.
.bien_taxon_suggestion_prebuilt <- local({
  data_dir <- file.path(getwd(), "data")
  read_one <- function(name) {
    f <- file.path(data_dir, name)
    if (file.exists(f)) {
      vals <- tryCatch(readRDS(f), error = function(e) NULL)
      if (is.character(vals) && length(vals) > 0) return(vals)
    }
    NULL
  }
  list(
    species = read_one("taxon_species.rds"),
    genus   = read_one("taxon_genus.rds"),
    family  = read_one("taxon_family.rds")
  )
})
local({
  built_on <- file.path(getwd(), "data", "taxon_lists_built_on.txt")
  if (file.exists(built_on)) {
    info <- tryCatch(readLines(built_on, warn = FALSE), error = function(e) character(0))
    if (length(info) > 0) message("Pre-built taxon lists: ", paste(info, collapse = "; "))
  } else {
    message("Pre-built taxon lists not found in data/; will fall back to live BIEN SQL.")
  }
})
...
load_taxon_suggestions <- function(rank, max_choices = 50000, timeout_sec = 120) {
  # Primary source: pre-built RDS shipped with the app (data/taxon_*.rds),
  # generated by tools/build_taxon_lists.R. Avoids runtime LIMIT truncation,
  # transient SQL failures, and silent demotion to the tiny hardcoded list.
  prebuilt <- .bien_taxon_suggestion_prebuilt[[rank]]
  if (!is.null(prebuilt) && length(prebuilt) > 0) {
    .bien_taxon_suggestion_cache[[rank]] <<- prebuilt
    return(prebuilt)
  }
