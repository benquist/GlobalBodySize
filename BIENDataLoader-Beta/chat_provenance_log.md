...
## 2026-04-27 — Hosted upload crash/disconnect hardening for user CSV files

**Prompt:** Implement a robust fix in `BIENDataLoader/app.R` for shinyapps.io disconnects when users upload their own CSV files.

**Summary:**
- Added `options(shiny.maxRequestSize = 100 * 1024^2)` near app startup so hosted uploads allow up to 100 MB per file.
- Added reusable `safe_read_csv_with_fallbacks()` that attempts CSV reads with delimiter/encoding fallbacks (`sep=","` and `";"`, `fileEncoding="UTF-8"` and `"latin1"`) while preserving `check.names=FALSE` and `stringsAsFactors=FALSE`.
- Replaced direct `read.csv()` calls for user-uploaded files with the safe helper in both primary upload ingestion and upload-back service CSV handlers (TNRS/GNRS/GVS/NSR).
- Hardened main upload observe path: if any uploaded file cannot be parsed, the app now catches the error, shows a clear `showNotification` message, clears `rv$raw_files`, and returns cleanly instead of allowing an unhandled exception to bubble up.
- Added explicit >100 MB rejection notification and kept existing >50 MB warning with updated wording that clarifies the 100 MB app limit.
- Preserved demo-data load path and downstream mapping/staging/service behavior.

**Validation:**
- `Rscript -e "parse(file='BIENDataLoader/app.R')"`
- Non-interactive helper smoke check run via `Rscript --vanilla -e '...safe_read_csv_with_fallbacks(...)...'` using temporary UTF-8 comma and latin1 semicolon CSV fixtures.
...
## 2026-04-27 — Memory-spike upload bug fix (5-fix optimizer patch)

**Prompt:** Production bug: user CSV upload shows "upload complete" then app disconnects. Demo data works. Root cause: memory spike from blank-row filtering via `as.matrix` on full uploaded frame.

**Summary:**
- FIX 1 (HIGH): Added `blank_row_filter()` helper (column-wise `lapply`, no full matrix copy; treats `"NA"` strings as blank). Replaced both `as.matrix` blank-row strips (demo data and user upload) with calls to this helper.
- FIX 2 (MEDIUM): Replaced `safe_read_csv_with_fallbacks()` with delimiter-sniffing version: reads first line to count commas vs semicolons, puts the winning separator first in the attempt list.
- FIX 3 (MEDIUM): Added aggregate 200 MB upload cap immediately after the per-file 100 MB rejection block.
- FIX 4 (MEDIUM): Added 20 MB early-return size guard after `req(input$upload_X)` in all four upload-back handlers (TNRS, GNRS, GVS, NSR).
- FIX 5 (LOW): Pre-hoisted `stg_names_key <- trimws(stg$scrubbed_species_binomial)` before both TNRS writeback loops to avoid repeated full-vector recompute per iteration.

**Validation:** `parse(file='BIENDataLoader/app.R')` → PARSE_OK; `blank_row_filter` smoke test → BLANK_FILTER_OK.
