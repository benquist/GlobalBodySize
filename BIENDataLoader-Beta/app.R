...
BIEN_STAGING_FIELDS <- c(
  ...
  # ── Plot Structure (beta: vegetation survey fields) ───────────────────────
  "cover",              # Percent cover of taxon in plot (0–100)
  "cover_total",        # Total vegetation cover in plot (0–100; stand-level)
  "relative_cover",     # Cover of taxon relative to all taxa in plot (0–100)
  "individual_count",   # Number of individuals of the taxon in plot
  "stem_count",         # Number of stems counted (may differ from individual_count for clonal taxa)
  "plot_area_ha",       # Plot surveyed area in hectares (preferred BIEN unit)
  "plot_size_m2",       # Plot area in m² (pass-through; convert to ha for BIEN ingest)
  "basal_area_m2ha",    # Stand basal area in m²/ha (plot-level aggregate)
  # ── Individual Plant Metrics ──────────────────────────────────────────────
  "height_m",           # Plant height in meters
  "dbh_cm",             # Diameter at breast height (1.3 m) in cm
  "stratum",            # Vertical stratum: canopy/subcanopy/shrub/herb/ground
  "subplot_name",       # Subplot or sub-quadrat identifier within the main plot
  # ── Topography ────────────────────────────────────────────────────────────
  "slope",              # Slope in degrees (0=flat, 90=vertical)
  "aspect",             # Aspect in degrees clockwise from N (0/360=N, 90=E, 180=S, 270=W)
  "topographic_position" # Qualitative: ridge/upper slope/mid slope/lower slope/valley
)
...
# ── Field definitions (beta: shown in Help tab reference table) ───────────────
BIEN_FIELD_DEFS <- c(
  ...
  # Plot structure
  cover                        = "Percent cover of the taxon within the plot (0-100). Key field for vegetation surveys.",
  cover_total                  = "Total vegetation cover in the plot (0-100); stand-level aggregate.",
  relative_cover               = "Cover of the taxon relative to all taxa in the plot (0-100).",
  individual_count             = "Number of individuals of the taxon recorded in the plot.",
  stem_count                   = "Number of stems counted (may differ from individual_count for clonal taxa).",
  plot_area_ha                 = "Plot surveyed area in hectares (preferred BIEN unit). Convert m2 / 10000.",
  plot_size_m2                 = "Plot area in square meters (pass-through; convert to ha for BIEN ingest).",
  basal_area_m2ha              = "Stand basal area in m2/ha (plot-level aggregate, not per-taxon).",
  # Individual plant metrics
  height_m                     = "Plant height in meters.",
  dbh_cm                       = "Diameter at breast height (measured at 1.3 m above ground) in centimeters.",
  stratum                      = "Vertical stratum of the individual: canopy, subcanopy, shrub, herb, or ground.",
  subplot_name                 = "Subplot or sub-quadrat identifier within the main plot.",
  # Topography
  slope                        = "Terrain slope in degrees from horizontal (0 = flat, 90 = vertical cliff).",
  aspect                       = "Terrain aspect in degrees clockwise from north (0/360=N, 90=E, 180=S, 270=W).",
  topographic_position         = "Qualitative topographic position: ridge, upper slope, mid slope, lower slope, or valley."
)
...
BIEN_ALIASES <- c(
  ...
  subplot = "subplot_name", sub_plot = "subplot_name", quadrat_id = "subplot_name",
  # ── Topography aliases ────────────────────────────────────────────────────
...
run_qc <- function(staged) {
  ...
  # ── Plot field range QC (beta: only fires for rows where field is populated) ──
    ...
    tags$div(class = "page-header",
      tags$div(class = "bien-header-brand",
        ...
        tags$div(
          ...
          tags$p("Upload, validate, and export BIEN-ready records \u2014 beta build (plot fields, verbatim names, field reference)")
        )
      )
    ),
...
server <- function(input, output, session) {
  ...
  # ── Help tab: Field Reference table (beta) ────────────────────────────────
