...
source(file.path(project_root, "providers", "fred", "R", "ingest_fred.R"), local = FALSE)
...
result <- ingest_fred_manifest(manifest_path = manifest_path, output_dir = output_dir)

message(sprintf(
  "FRED ingest complete: %s dataset rows, %s file rows.",
  result$dataset_rows,
  result$file_rows
))
