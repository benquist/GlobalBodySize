...
source(file.path(project_root, "providers", "leda", "R", "ingest_leda.R"), local = FALSE)
...
result <- ingest_leda_manifest(manifest_path = manifest_path, output_dir = output_dir)

message(sprintf(
  "LEDA ingest complete: %s dataset rows, %s file rows.",
  result$dataset_rows,
  result$file_rows
))
